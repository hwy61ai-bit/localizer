...
function buildPreviewUrl(publicId: string, cloudName: string, cfg: FormatConfig, format: FormatKey): string {
  ...
  const layers = [
    ...
    `co_rgb:${color},fl_layer_apply,g_center,l_text:${font}_${cfg.venue.size}_bold:${san("Stubbs Waller Creek Amphitheater")},x_${vp.xPx},y_${vp.yPx}`,
    `co_rgb:${color},fl_layer_apply,g_center,l_text:${font}_${cfg.date.size}:${san("April 25 2026")},x_${dp.xPx},y_${dp.yPx}`,
    `co_rgb:${color},fl_layer_apply,g_center,l_text:${font}_${cfg.city.size}:${san("Little Rock AR")},x_${cp.xPx},y_${cp.yPx}`,
  ];
