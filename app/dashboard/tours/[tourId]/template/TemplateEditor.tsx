...
function buildPreviewUrl(publicId: string, cloudName: string, cfg: FormatConfig, format: FormatKey): string {
  ...
  const layers = [
    ...
    `c_fit,co_rgb:${color},fl_layer_apply,g_center,l_text:${font}_${Math.round(cfg.venue.size * scale)}_bold:${san("Stubbs Waller Creek Amphitheater")},w_${maxW},x_${vp.xPx},y_${vp.yPx}`,
    `c_fit,co_rgb:${color},fl_layer_apply,g_center,l_text:${font}_${Math.round(cfg.date.size * scale)}:${san("April 25 2026")},w_${maxW},x_${dp.xPx},y_${dp.yPx}`,
    `c_fit,co_rgb:${color},fl_layer_apply,g_center,l_text:${font}_${Math.round(cfg.city.size * scale)}:${san("Little Rock AR")},w_${maxW},x_${cp.xPx},y_${cp.yPx}`,
  ];
