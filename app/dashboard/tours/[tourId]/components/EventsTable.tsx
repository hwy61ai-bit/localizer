...
function buildUrl(publicId: string, cloudName: string, cfg: FormatConfig, event: EventRow, w: number, h: number): string {
  ...
  const layers = [
    ...
    `co_rgb:${color},fl_layer_apply,g_center,l_text:${font}_${Math.round(cfg.venue.size * scale)}_bold:${venueName},x_${vp.xPx},y_${vp.yPx}`,
    `co_rgb:${color},fl_layer_apply,g_center,l_text:${font}_${Math.round(cfg.date.size * scale)}:${dateStr},x_${dp.xPx},y_${dp.yPx}`,
    `co_rgb:${color},fl_layer_apply,g_center,l_text:${font}_${Math.round(cfg.city.size * scale)}:${cityState},x_${cp.xPx},y_${cp.yPx}`,
  ];
