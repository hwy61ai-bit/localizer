...
export async function POST(req: NextRequest) {
  ...
  const { data: tour, error: tourError } = await supabase
    .from("tours")
    .select("id, org_id, name, band_name, band_tour_label, image_url, image_print_id, image_square_id, image_story_id, image_landscape_id, video_tiktok_id, video_yt_shorts_id, overlay_config, sponsor_logo_1_url, sponsor_logo_2_url, custom_text_1, custom_text_2")
    .eq("id", tourId)
    .eq("org_id", orgId)
    .single();
