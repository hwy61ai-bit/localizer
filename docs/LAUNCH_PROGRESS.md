...
- **Blocked on:** Tim's final Terms/Privacy text file (gates the legal deploy — see Active blockers); Tim's video script review (Day 12); Tim's live Stripe verification screen-share; the in-progress legal-handoff build (Drew, ~1.5–2 days remaining — W-9 build and visibility toggles shipped July 3).
...
| Legal-handoff build (Drew) | Drew | In progress — ~1.5–2 working days remaining. ✅ B1 shipped July 2 (private advance-docs bucket + `advance_doc_access_log` table). ✅ W-9 build B2→B3→B4 shipped July 3 (private upload route, gated download route with W-9 acknowledgment gate + verified access log, file migration complete — 9 W-9s + 0 remaining fixed advance docs public). ✅ Visibility toggles for team contacts / advance docs / W-9 shipped July 3 afternoon (three per-artist booleans, server-side payload-level gating, HwToggle UI). Remaining: ⬜ signup assent checkbox (0.5d); ⬜ cookie consent banner with pre-consent analytics off (1d); ⬜ legal text deploy + launch-day snapshot (0.5d, gated on Tim's final text — see row above). Runs in parallel with Tim's text finalization. |
