...
**One open non-blocking Tim question:** should the opener draw on the Local Poster Print PDF? Recommendation: yes — opener is per-show info like venue/date/city (all three draw on print, unlike tour-level custom text which is excluded). Ships as Phase 1b if approved; not blocking Phase 1 image/video scope.
