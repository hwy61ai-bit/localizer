...
- **Blocked on:** Tim's final Terms/Privacy text file (gates the legal deploy — see Active blockers); Tim's video script review (Day 12); Tim's live Stripe verification screen-share; the in-progress legal-handoff build (Drew, ~5–7 days, B1 shipped July 2).
...
| Legal-handoff build (Drew) | Drew | In progress — ~2.5–3.5 working days remaining. ✅ B1 shipped July 2 (private advance-docs bucket + `advance_doc_access_log` table). ✅ W-9 build B2→B3→B4 shipped July 3 (private upload route, gated download route with W-9 acknowledgment gate + verified access log, file migration complete — 9 W-9s + 0 remaining fixed advance docs public). Remaining: ⬜ visibility toggles for team contacts / advance docs / W-9 (1–2d); ⬜ signup assent checkbox (0.5d); ⬜ cookie consent banner with pre-consent analytics off (1d); ⬜ legal text deploy + launch-day snapshot (0.5d, gated on Tim's final text — see row above). Runs in parallel with Tim's text finalization. |
