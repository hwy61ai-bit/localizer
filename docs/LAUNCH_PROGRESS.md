...
| Legal-handoff build (Drew) | Drew | In progress — ~5–7 working days total. B1 shipped July 2 (private advance-docs bucket + `advance_doc_access_log` table). Remaining: B2 server upload route → B3 gated download + confirmation modal + access log → B4 file migration (1–1.5d together); visibility toggles for team contacts / advance docs / W-9 (1–2d); signup assent checkbox (0.5d); cookie consent banner with pre-consent analytics off (1d); legal text deploy + launch-day snapshot (0.5d, gated on Tim's final text — see row above). Runs in parallel with Tim's text finalization. |
