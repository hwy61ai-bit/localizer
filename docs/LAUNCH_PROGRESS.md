...
- **Blocked on:** Tim's video script review (Day 12); Tim's live Stripe verification screen-share; Erin's (attorney) review of ToS/Privacy (PDF sent to Tim June 15 to forward — items 1–3 plus DMCA/IP/marketing-claim/cookie/indemnity questions); the Privacy Policy §3 rewrite for venue-page PII exposure (Erin finding 2.1, mandatory pre-launch — see Active blockers).
...
| Attorney (Erin) review of ToS/Privacy | Tim → Erin | Review PDF (`docs/legal/Localizer_Legal_Review_for_Erin.pdf`, 7 findings) built June 15, sent to Tim to forward to Erin. Items 1–3 (governing law/arbitration, liability cap $0 floor for trial users, multi-state/GDPR thresholds) plus DMCA §512(g) counter-notice, IP reps, marketing-claim review, cookie banner, indemnity carve-out all await Erin's answers. Longest-pole launch item. |
| Privacy Policy §3 rewrite — venue-page PII exposure | Drew (pending Erin's bar) | June 14 Team/Marketing build put third-party PII on public tokenized venue URLs (`/v/e/[token]`): team contact names/emails/phones (Manager, Tour Manager, Booking Agent, Publicist + custom roles), Meta Business Manager ID, W-9 + advance-material download links. Privacy §3 still characterizes venue links as exposing "no personal info" — now inaccurate. §3 rewrite is MANDATORY pre-launch (Erin finding 2.1). Per-field "show on venue page" opt-in toggle is the fast-follow (in BACKLOG Future ideas). Flagged as the item most likely to move the launch date. |
