...
- **Blocked on:** Tim — demo video script v2 review, final Terms/Privacy text file, Stripe live-verification screen-share.
...
| Tim's final Terms/Privacy text file | Tim | Gates the legal-text deploy — the last step of the legal-handoff build. Tim's July 2 handoff (`docs/DREW_HANDOFF_LEGAL_LAUNCH_SCOPE_2026-07-02.md`) decided all seven Erin-review items and set the direction to launch ahead of the Erin meeting; the final rewritten Terms + Privacy text is the deliverable that unblocks the on-site deploy. Supersedes the prior "Attorney (Erin) review" and "Privacy Policy §3 rewrite" rows. |
| Legal-handoff build (Drew) | Drew | Drew's build queue is empty as of July 6. ✅ B1 shipped July 2 (private advance-docs bucket + `advance_doc_access_log` table). ✅ W-9 build B2→B3→B4 shipped July 3. ✅ Visibility toggles for team contacts / advance docs / W-9 shipped July 3. ✅ Signup assent checkbox shipped July 6 (`619910f`) — required checkbox on `/login`, `user_assents` table live (RLS, service-role-only), stamped at first login via `ensureOrgExists`. ✅ Cookie consent banner shipped July 6 (`670847f`) — PostHog init gated on affirmative consent, Accept/Decline banner in root layout, choice persisted in `localStorage`; prod network-tab verification pending Vercel deploy. Remaining: ⬜ legal text deploy + launch-day snapshot (0.5d, gated on Tim's final text — see row above). |
