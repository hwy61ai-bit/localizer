...
- **32 items added since the original plan was written** (see "Added since the original plan" section)
...
- **🚨 NEXT SESSION (start here): watermark hero copy fix.** Live `/` hero still says "1 artist, watermarked" — we no longer watermark anything (the watermark / shows-per-month / feature-gate model was cut May 28 and replaced by the no-card 7-day trial). Change to "7-day free trial, full access." Stale-wrong copy on the highest-traffic public surface. ~5 min. Has been deferred multiple sessions — do it first.
- **NEXT SESSION: venue-link page Share/copy-link button.** Add a "Share"/copy-link button to `app/v/e/[token]/page.tsx` directly below the existing Download All button — copies the venue link URL to clipboard, shows "Copied" on click. Simple client-side `navigator.clipboard.writeText` pattern; mirrors the existing copy pattern in `ShareWithMarketingButton.tsx` (`handleCopy`). Quick win for the venue viewer (which a promoter sees, so it's customer-facing UX, not internal).
